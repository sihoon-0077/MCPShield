...
test("OCI console uses actual API/BFF preparation, strict export and explicit synthetic AI scope without crossing npm policy or reader permissions", { timeout: 20000 }, async () => {
  ...
  try {
    ...
    const missing = renderToStaticMarkup(React.createElement(SemanticEvidenceNotice, { required: true })); assert.match(missing, /미제공/); assert.doesNotMatch(missing, /LOCAL_CONTRACT_TEST/);
    for (const semanticEvidenceMode of [undefined, "UNKNOWN_API_MODE"]) {
      ...
      assert.match(unknown, /모델 품질은 미측정/); assert.match(unknown, /분석 모드를 임의로 추정하지 않습니다/); assert.doesNotMatch(unknown, /로컬 합성 응답으로 분석 연동을 검사/);
    }
