...
Set `MCPSHIELD_API_URL` for LIVE, `MCPSHIELD_SCAN_IDS` to a comma-separated safe/malicious scan pair, `MCPSHIELD_REPLAY_FILE` for an alternate replay bundle, and optionally `MCPSHIELD_EXPLORER_URL` for LIVE transaction links. Unconfigured or invalid explorer URLs are never rendered as links.
