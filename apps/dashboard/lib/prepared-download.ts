...
// @ts-expect-error OCI uses its own strict native descriptor/execution-policy commitment.
...
export function preparedDownload(payload: any, releaseId: string) {
  const validBinding = payload?.binding?.profile === "oci-container-v1" ? validateOciReleaseBinding : validatePreparedReleaseBinding;
  if (!payload || Object.keys(payload).sort().join() !== "binding,releaseId,schemaVersion,toolId,tools" ||
    ...
    !/^0x[a-f0-9]{64}$/.test(payload.toolId) || !validBinding(payload.binding) || !Array.isArray(payload.tools) || payload.tools.length > 128 ||
