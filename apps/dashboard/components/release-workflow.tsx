...
export function ReleaseWorkflow({ release, scans, policies, actions, manage, onRefresh }: {
  ...
  return <section className="ops-workflow" aria-label="V2 검증에서 실행 판정까지">
    ...
    {manage && <details className="ops-chain-admin"><summary>관리자 · 온체인 등록 요청</summary><p>설정된 서버 relayer가 트랜잭션을 전송합니다. 네트워크 가스 비용이 발생할 수 있습니다. 검증자 투표를 대신하지 않습니다.</p><form onSubmit={enqueue}><label>등록할 대상<select name="chainOperation" disabled={busy}><option value="release">현재 릴리스 등록</option>{policy && !policy.deprecatedAt && <option value="policy">선택 정책 공개 · {policy.alias}</option>}</select></label><label className="ops-check"><input type="checkbox" required disabled={busy} />위 릴리스/정책과 서버에 설정된 체인으로 전송을 요청합니다.</label><button disabled={busy}>온체인 등록 요청</button></form></details>}
