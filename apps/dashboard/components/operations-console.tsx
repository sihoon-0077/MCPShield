...
import { ReleaseWorkflow, type Release, type Scan, type ChainAction } from "./release-workflow";
...
export function OperationsConsole() {
  ...
  async function refresh() {
    const [nextReleases, nextScans, nextPolicies, nextOperations, nextActions] = await Promise.all([api<{ items: Release[] }>("releases"), api<{ items: Scan[] }>("scans"), api<{ items: Policy[] }>("policies"), api<Operations>("operations"), api<{ items: ChainAction[] }>("chain/actions")]);
    setReleases(nextReleases.items); setScans(nextScans.items); setPolicies(nextPolicies.items);
    setOperations(nextOperations);
    setChainActions(nextActions.items);
    if (selected) {
      const [events, requests] = await Promise.all([api<{ items: History[] }>(`releases/${encodeURIComponent(selected)}/history`), api<{ items: Appeal[] }>(`releases/${encodeURIComponent(selected)}/appeals`)]);
  ...
  return <main className="ops" lang="ko">
    ...
    {!session ? <section className="ops-panel ops-login"><h2>조직에 연결</h2><p>관리자가 발급한 운영 액세스 토큰을 입력하세요. 계정에 지정된 조직과 역할만 접근할 수 있습니다.</p><form onSubmit={(event) => submit(event, async (data) => { const current = await api<Session>("session", { token: field(data, "token") }); setSession(current); await refresh(); })}><label>액세스 토큰<input name="token" type="password" required minLength={16} maxLength={2048} autoComplete="off" /></label><button disabled={busy}>운영 콘솔 연결</button></form><small>토큰은 JavaScript에서 읽을 수 없는 HttpOnly 쿠키로 보관하며, 주소나 localStorage에 기록하지 않습니다.</small></section> : <>
      ...
      {session.capabilities.scan && <section className="ops-panel"><h2>새 릴리스 등록</h2><p>정확한 버전을 입력하세요. 다운로드된 파일의 digest로 릴리스를 고정합니다.</p><form className="ops-inline-form" onSubmit={(event) => submit(event, async (data) => { await api("releases/resolve", { sourceType: field(data, "sourceType"), locator: field(data, "locator") }); await refresh(); })}><label>가져올 위치<select name="sourceType"><option value="npm">npm 패키지</option><option value="tarball">Tarball URL</option><option value="oci">OCI 이미지 (digest 고정)</option><option value="fixture">로컬 데모 fixture</option></select></label><label>정확한 패키지 또는 주소<input name="locator" required maxLength={2048} placeholder="패키지@1.0.0 / mail-mcp-1.0.0" /></label><button disabled={busy}>릴리스 등록</button></form></section>}
