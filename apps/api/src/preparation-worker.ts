...
async function checkedInput(store: ControlStore, job: PreparationJob) {
  ...
  if (!policy || policy.deprecatedAt || !validPolicy(policy.document) || hash(policy.document) !== job.policyHash || !(source.sourceType === "oci" ? policy.document.profile === ociPolicy.profile : isNodePreparedPolicy(policy.document))) throw new Error("PREPARATION_POLICY_UNAVAILABLE");
...
export async function runPreparationWorkerOnce(store: ControlStore, options: ControlOptions, owner: string = randomUUID()) {
  ...
  try {
    ...
    if (output.binding) {
      ...
      if (scoped) {
        if (hash(binding.executionPolicy) !== hash(scoped.scopedReview.executionPolicy)) throw Error("SCOPED_EXECUTION_POLICY_MISMATCH");
    ...
    const committed = await withSpan("scan.accept", {}, () => store.forTenant(job.tenantId, async (tx) => {
      ...
      const result = { outcome: derived ? "DERIVED_RELEASE_CREATED" : "INCONCLUSIVE", ...(derived ? { releaseId: derived.releaseId, scanId, verdict: scanResult!.verdict } : { verdict: "ABSTAIN" }),
        ...
        ...(oci ? { semanticEvidenceMode: policy.document.semanticEvidenceMode, providerQuality: "PROVIDER_QUALITY_NOT_MEASURED" } : scopedMetadata(policy.document)) };
...
export async function scanPreparedRelease(scan: ScanJob, release: Record<string, any>, options: ControlOptions, policy?: any, sourceRelease?: Record<string, any>) {
  ...
  const output = await execute({ descriptor: binding.descriptor, expectedDescriptorDigest: binding.descriptorDigest, sourceReleaseId: binding.sourceReleaseId,
    releaseId: release.legacyReleaseId, scanId: scan.scanId, ai: scoped?.ai ?? preparedAi(options),
    ...(scoped ? { scopedReview: scoped.scopedReview } : {}), ...(oci ? { trust: localOci } : { trusted: runtimeTrust }) });
