...
export function checkedPreparedEvidence(bundle: any, identity?: any) {
  const binding = JSON.parse(bundle.files["prepared/binding.json"] ?? "null"), source = JSON.parse(bundle.files["prepared/source-identity.json"] ?? "null");
  const tools = JSON.parse(bundle.files["runtime/tools.json"] ?? "null");
  if (!validatePreparedReleaseBinding(binding) || !source || Object.keys(source).sort().join() !== "artifactDigest,manifestDigest,releaseId,toolId,toolSurfaceHash"
    ...
    || canonical(JSON.parse(bundle.files["runtime/descriptor.json"] ?? "null")) !== canonical(binding.descriptor)
