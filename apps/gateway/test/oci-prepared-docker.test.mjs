...
import { createOciProfileFixture, checkedAuthoredOciScript } from "../../../tests/security/oci-profile-fixture.mjs";
...
async function docker(args) {
  try { const result = await execute("docker", args, { timeout: 10000, maxBuffer: 128 * 1024, encoding: "utf8", windowsHide: true }); return result.stdout.trim(); }
...
const authoredScript = ["#!/bin/sh", "linger=0", "while IFS= read -r line; do",
...
test("shared OCI source preserves Gateway's authored native isolation, string-ID pagination, private stderr and lingering fixture", () => {
  assert.equal(checkedAuthoredOciScript(authoredScript), authoredScript);
  for (const check of ["id -u", "/sys/class/net", "CapEff:", "NoNewPrivs:", "Seccomp:", "/must-not-write", "MCPSHIELD_CONTROL_TOKEN", "PYTHONDONTWRITEBYTECODE", "SYNTHETIC_CANDIDATE_STDERR_PRIVATE", "sleep 3600", '"cursor":"second"']) assert.ok(authoredScript.includes(check), check);
  assert.ok(authoredScript.includes('"id":\\([^,}]*\\)'));
});

// The common source fixture exports a never-started approved config ID, without
// BuildKit FROM lookup, and adds only our authored script. No host candidate execution.
// Admission signer/7 review anchors remain synthetic, not independent quorum evidence.
test("actual OCI native shell → source import/observation → signed Gateway enforces isolation and per-call revocation", {
  ...
  const root = await mkdtemp(join(tmpdir(), "mcpshield-oci-gateway-native-")), before = await ownedContainers();
  let imported, server, fixture;
  try {
    fixture = await createOciProfileFixture({ builderImageDigest: builder, authoredScript });
    const { root: source, sourceTreeDigest, platform } = fixture;
    imported = await importOciRuntime({ root: source, sourceTreeDigest, platform });
    ...
    try { await imported?.cleanup?.(); }
    finally { try { await fixture?.cleanup(); } finally { await removeFixtureSnapshot(root); } }
    assert.deepEqual(await ownedContainers(), before);
  }
});
