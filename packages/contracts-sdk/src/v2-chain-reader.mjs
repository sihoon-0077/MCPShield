      ...
      for (const { provider, registry } of readers) {
        ...
        for (;;) {
          try {
            ...
            if (controller.signal.aborted) throw new Error("RPC_BUDGET_EXCEEDED");
            ...
            if (error?.message === "CHAIN_VIEW_CHANGED") throw new Error("STATUS_UNAVAILABLE");
            if (error === FRESH_VIEW) {
              if (freshViewRetries-- > 0 && !controller.signal.aborted) continue;
              throw new Error("STATUS_UNAVAILABLE");
            }
            break; // Configured RPC alternatives share the original deadline.
          }
        }
      }
      throw new Error("STATUS_UNAVAILABLE");
