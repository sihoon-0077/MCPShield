...
function descriptor(value) {
  if (!value || !digestPattern.test(value.digest) || !Number.isSafeInteger(value.size) || value.size < 0 || value.size > SNAPSHOT_LIMITS.bytes) throw new TypeError('invalid or oversized OCI descriptor');
  if (value.urls?.length) throw new TypeError('foreign OCI layer URLs are not allowed');
  return value;
}

function verifyBlob(bytes, expected) {
  descriptor(expected);
...
export async function inspectOciImage({ index, readBlob, platform = { os: 'linux', architecture: 'amd64' } }) {
  ...
  const configBytes = verifyBlob(await readBlob(descriptor(selected.config)), selected.config);
  ...
  for (const layer of selected.layers) {
    ...
    if (layerBytes > SNAPSHOT_LIMITS.bytes) throw new Error('OCI_LAYER_SIZE_LIMIT');
    verifyBlob(await readBlob(layer), layer);
...
export async function resolveOciArtifact(source) {
  ...
  try {
    ...
    if (source.type === 'oci-layout') {
      await copyFixtureSnapshot(source.path ?? source.locator, layoutDir);
      const layout = JSON.parse(await readFile(join(layoutDir, 'oci-layout'), 'utf8'));
      if (layout.imageLayoutVersion !== '1.0.0') throw new TypeError('unsupported OCI layout version');
      index = JSON.parse(await readFile(join(layoutDir, 'index.json'), 'utf8'));
      readBlob = async (entry) => readFile(join(layoutDir, 'blobs', 'sha256', descriptor(entry).digest.slice(7)));
      ...
      let bearer;
      const registryGet = async (path, accept) => {
      ...
      readBlob = async (entry) => {
        ...
        verifyBlob(bytes, entry);
        await writeFile(join(layoutDir, 'blobs', 'sha256', entry.digest.slice(7)), bytes);
        return bytes;
      };
    }
    const inspection = await inspectOciImage({ index, readBlob, platform: source.platform });
    ...
    const treeDigest = await artifactDigest(artifactDir);
